    ...
    <table cellspacing="0" class="a-admin-list-table">
      <thead>
        <tr>
					          	...
					          	<?php include_partial('aGroupAdmin/list_th_tabular', array('sort' => $sort, 'helper' => $helper)) ?>
      ...
      <tfoot>
        <tr>
          <th colspan="5">
            ...
            <?php if ($pager->haveToPaginate()): ?>
              <?php include_partial('aGroupAdmin/pagination', array('pager' => $pager)) ?>
            <?php endif; ?>	
          </th>
        </tr>
      </tfoot>
      <tbody>
        <?php $n=1; $total = $pager->getNbResults(); foreach ($pager->getResults() as $i => $sf_guard_group): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr class="a-admin-row <?php echo $odd ?> <?php echo ($n == $total)? 'last':'' ?> <?php echo ($n == 1)? 'first':'' ?>">
						            	<?php include_partial('aGroupAdmin/list_td_batch_actions', array('sf_guard_group' => $sf_guard_group, 'helper' => $helper)) ?>
						            	<?php include_partial('aGroupAdmin/list_td_tabular', array('sf_guard_group' => $sf_guard_group, 'helper' => $helper)) ?>
						            	<?php include_partial('aGroupAdmin/list_td_actions', array('sf_guard_group' => $sf_guard_group, 'helper' => $helper)) ?>
						          </tr>
        <?php $n++; endforeach; ?>
      </tbody>
    </table>
